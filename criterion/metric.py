...
def compute_area(y: list, unfold: nn.Unfold, area: torch.Tensor) -> torch.Tensor:
    ...
    yy = torch.stack(y, dim=0).to(torch.float16).unsqueeze(0) # bit (0/1) but unfold requires float
    
    # unfold slides through the volume like a convolution
    # 2x2 kernel returns 8 values (2 channels * 2x2)

    cubes_float = unfold(yy).squeeze()  # (8, n_cubes)
    ...
    cubes_area = area[cubes_byte.long()]

    return cubes_area


def score(submit: pd.DataFrame, label: pd.DataFrame) -> float:
    """
...
def surface_dice(pred, target, thresh=0.1):
    if torch.any(pred<0) or torch.any(pred>1):
        pred=pred.sigmoid()
    pred=pred.reshape(-1)>thresh
